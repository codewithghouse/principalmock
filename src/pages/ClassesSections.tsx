  ...
  return (
    <div className={isMobile ? "animate-in fade-in duration-500" : "space-y-8 animate-in fade-in duration-500 pb-10"}>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-normal text-slate-900 tracking-tight">Classes & Sections</h1>
          <p className="text-sm text-slate-400 font-normal mt-1">Overview of all classes and sections</p>
        </div>
        <button
          onClick={() => setAddModal(true)}
          className="flex items-center gap-2 px-6 py-3 bg-[#1D1D1F] text-white rounded-xl text-sm font-normal hover:bg-[#0A84FF] transition-colors shadow-md"
        >
      ...
      {loading ? (
        <div className="py-32 flex flex-col items-center justify-center bg-white rounded-2xl border border-slate-100 shadow-sm">
          ...
          <p className="text-[12px] font-normal text-slate-400 uppercase tracking-widest">Loading Classes...</p>
        </div>
      ) : (
        <>
          {/* Grade Summary Cards */}
                ...
                return (
                  <div key={g.grade} className="bg-white border border-slate-100 rounded-xl p-3.5 shadow-sm hover:shadow-md transition-all">
                    <div className="flex items-center justify-between mb-2.5">
                      <h3 className="text-sm font-normal text-slate-900">Grade {g.grade}</h3>
                      <Icon className={`w-4 h-4 ${healthColor(g.healthScore)}`} />
                    </div>
                    <div className="space-y-1.5 text-[12px]">
                      <div className="flex justify-between">
                        <span className="text-slate-400 font-normal">Sections</span>
                        <span className="font-normal text-slate-900">{g.sections}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-400 font-normal">Students</span>
                        <span className="font-normal text-slate-900">{g.students}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-400 font-normal">Avg Attendance</span>
                        <span className={`font-normal ${g.avgAttendance >= 85 ? "text-green-600" : g.avgAttendance >= 70 ? "text-amber-500" : "text-rose-600"}`}>
                          {g.avgAttendance > 0 ? `${g.avgAttendance}%` : "—"}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-400 font-normal">Health Score</span>
                        <span className={`font-normal ${healthColor(g.healthScore)}`}>
          ...
          <div className="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm">
            <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
              <h2 className="text-base font-normal text-slate-900">Section Performance</h2>
              <span className="text-xs text-slate-400 font-normal">{classes.length} class{classes.length !== 1 ? "es" : ""}</span>
            </div>
            ...
            {classes.length === 0 ? (
              <div className="py-10 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-sm font-normal text-slate-400 uppercase tracking-widest">No classes found</p>
                ...
                <table className="w-full text-sm text-left min-w-[700px]">
                  <thead>
                    <tr className="bg-slate-50/50 border-b border-slate-100">
                      <th className="px-6 py-4 text-[12px] font-normal text-slate-400 uppercase tracking-widest">Section</th>
                      <th className="px-6 py-4 text-[12px] font-normal text-slate-400 uppercase tracking-widest">Class Teacher</th>
                      <th className="px-6 py-4 text-[12px] font-normal text-slate-400 uppercase tracking-widest text-center">Students</th>
                      <th className="px-6 py-4 text-[12px] font-normal text-slate-400 uppercase tracking-widest text-center">Avg Marks</th>
                      <th className="px-6 py-4 text-[12px] font-normal text-slate-400 uppercase tracking-widest text-center">Attendance</th>
                      <th className="px-6 py-4 text-[12px] font-normal text-slate-400 uppercase tracking-widest">Weak Subject</th>
                      <th className="px-6 py-4 text-[12px] font-normal text-slate-400 uppercase tracking-widest">Status</th>
                      <th className="px-6 py-4 text-[12px] font-normal text-slate-400 uppercase tracking-widest text-right">Actions</th>
                    </tr>
                  </thead>
                        ...
                        <tr key={cls.id} className="hover:bg-slate-50/30 transition-colors group">
                          ...
                          <td className="px-6 py-5">
                            <div className="flex items-center gap-3">
                              <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-white text-xs font-normal shrink-0 shadow-sm ${
                              ...
                              <div>
                                <p className="font-normal text-slate-900">{cls.name}</p>
                                {cls.subject && <p className="text-[12px] text-slate-400 font-normal mt-1">{cls.subject}</p>}
                              </div>
                            </div>
                          </td>
                            ...
                            {cls.teacherName ? (
                              <div className="flex items-center gap-2 flex-wrap">
                                <div className="flex items-center gap-1.5">
                                  ...
                                  <span className="text-sm font-normal text-slate-700">{cls.teacherName}</span>
                                </div>
                                <button
                                  onClick={() => { setAssigningClass(cls); setAssignTeacherId(cls.teacherId || ""); setAssignModal(true); }}
                                  className="text-[12px] text-blue-500 hover:text-blue-700 font-normal underline underline-offset-2"
                                >
                                  Change
                                </button>
                              </div>
                              ...
                              <button
                                ...
                                className="flex items-center gap-1.5 text-[12px] text-amber-700 bg-amber-50 border border-amber-200 px-3 py-1.5 rounded-lg font-normal hover:bg-amber-100 transition-colors"
                              >
                          ...
                          <td className="px-6 py-5 text-center">
                            <div className="flex items-center justify-center gap-1.5">
                              ...
                              <span className="font-normal text-slate-900">{cls.studentCount}</span>
                            </div>
                          </td>
                          ...
                          {/* Weak Subject */}
                          <td className="px-6 py-5">
                            <span className={`text-sm font-normal ${cls.weakSubject !== "—" ? "text-rose-500" : "text-slate-300"}`}>
                              {cls.weakSubject}
                            </span>
                          </td>
                          ...
                          <td className="px-6 py-5">
                            <div className="flex items-center gap-2">
                              ...
                              <span className={`text-sm font-normal ${statusColor(cls.status)}`}>{cls.status}</span>
                            </div>
                          </td>
                          ...
                          <td className="px-6 py-5 text-right">
                            <div className="flex items-center justify-end gap-2">
                              <button
                                ...
                                className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-indigo-200 bg-indigo-50 text-indigo-700 text-[12px] font-normal hover:bg-indigo-100 transition-colors"
                              ...
                              <button
                                ...
                                className="px-5 py-2 rounded-xl bg-slate-900 text-white text-[12px] font-normal uppercase tracking-wider hover:bg-[#1D1D1F] transition-colors shadow-sm"
                              >
                                View
                              </button>
                            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <h3 className="text-base font-normal text-slate-900">Add New Class</h3>
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="text-xs font-normal text-slate-500 uppercase tracking-widest block mb-2">Class Name *</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-normal text-slate-500 uppercase tracking-widest block mb-2">Grade *</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-xs font-normal text-slate-500 uppercase tracking-widest block mb-2">Section</label>
                  <input
                    type="text"
                    placeholder="e.g. A, B"
              ...
              <div>
                <label className="text-xs font-normal text-slate-500 uppercase tracking-widest block mb-2">Subject (optional)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-xs font-normal text-slate-500 uppercase tracking-widest block mb-2">Assign Class Teacher (optional)</label>
                <select
                  value={newClassTeacherId}
                  onChange={e => setNewClassTeacherId(e.target.value)}
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm font-normal focus:outline-none focus:ring-2 focus:ring-[#1D1D1F]/20 bg-white appearance-none"
                >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-between p-6 border-b border-slate-100 shrink-0">
              <div>
                <h3 className="text-base font-normal text-slate-900 flex items-center gap-2">
                  <UserPlus className="w-4 h-4 text-[#1D1D1F]" /> Add Students to {studentModalClass.name}
                </h3>
            ...
            <div className="flex border-b border-slate-100 shrink-0">
              <button onClick={() => setStudentTab("existing")} className={`flex-1 py-3 text-sm font-normal transition-colors ${studentTab === "existing" ? "text-[#1D1D1F] border-b-2 border-[#1D1D1F]" : "text-slate-400 hover:text-slate-600"}`}>
                From School List
              </button>
              <button onClick={() => setStudentTab("invite")} className={`flex-1 py-3 text-sm font-normal transition-colors ${studentTab === "invite" ? "text-[#1D1D1F] border-b-2 border-[#1D1D1F]" : "text-slate-400 hover:text-slate-600"}`}>
                Invite New Student
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 min-h-0">
              {studentTab === "existing" ? (
                    ...
                    return filtered.length === 0 ? (
                      ...
                      <div className="space-y-2">
                        {selectedSids.length > 0 && (
                          <p className="text-xs font-normal text-[#1D1D1F] mb-1">{selectedSids.length} selected</p>
                        )}
                        {filtered.map((s: any) => {
                          const isSel = selectedSids.includes(s.id);
                          return (
                            <div key={s.id} onClick={() => setSelectedSids(prev => isSel ? prev.filter(id => id !== s.id) : [...prev, s.id])}
                              className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all border ${isSel ? "bg-blue-50 border-[#1D1D1F]/30" : "border-slate-100 hover:bg-slate-50"}`}
                            >
                              <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center shrink-0 transition-all ${isSel ? "bg-[#1D1D1F] border-[#1D1D1F]" : "border-slate-300"}`}>
                                {isSel && <Check className="w-3 h-3 text-white" />}
                              </div>
                              <div className="w-9 h-9 rounded-xl bg-indigo-50 flex items-center justify-center text-xs font-normal text-indigo-600 shrink-0">
                                {(s.name || "S").substring(0, 2).toUpperCase()}
                              </div>
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-normal text-slate-800 truncate">{s.name || "Unknown"}</p>
                                <p className="text-xs text-slate-400 truncate">{s.email}</p>
                              </div>
                              {s.className && s.className !== studentModalClass.name && (
                                <span className="text-[12px] font-normal text-slate-400 bg-slate-100 px-2 py-0.5 rounded-md shrink-0">{s.className}</span>
                              )}
                            </div>
                          );
                        })}
                ...
                <div className="space-y-4">
                  <div>
                    <label className="text-xs font-normal text-slate-500 uppercase tracking-widest block mb-2">Student Name *</label>
                  ...
                  <div>
                    <label className="text-xs font-normal text-slate-500 uppercase tracking-widest block mb-2">Email Address *</label>
            ...
            <div className="flex gap-3 p-6 border-t border-slate-100 shrink-0">
              <button onClick={() => setStudentModal(false)} className="flex-1 py-3 rounded-xl border border-slate-200 text-sm font-normal text-slate-600 hover:bg-slate-50">
                Cancel
              </button>
              {studentTab === "existing" ? (
                <button onClick={handleAddExistingToClass} disabled={enrolling || selectedSids.length === 0}
                  className="flex-1 py-3 rounded-xl bg-[#1D1D1F] text-white text-sm font-normal hover:bg-blue-800 disabled:opacity-60 flex items-center justify-center gap-2"
                >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-6 border-b border-slate-100">
              <div>
                <h3 className="text-base font-normal text-slate-900 flex items-center gap-2">
                  <UserPlus className="w-4 h-4 text-[#1D1D1F]" />
                  {assigningClass?.teacherName ? "Change Class Teacher" : "Assign Class Teacher"}
                </h3>
                <p className="text-xs text-slate-400 mt-1 font-normal">{assigningClass?.name}</p>
              </div>
              <button
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="text-xs font-normal text-slate-500 uppercase tracking-widest block mb-2">Select Teacher *</label>
                {teachers.length === 0 ? (
                  <div className="w-full bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm font-normal text-amber-700">
              ...
              {assigningClass?.teacherName && (
                <p className="text-xs text-slate-400 bg-slate-50 rounded-lg px-3 py-2">
                  Current teacher: <span className="font-normal text-slate-600">{assigningClass.teacherName}</span>
                </p>
              )}
              <div className="flex gap-3 pt-2">
                <button
