    ...
    <div className="my-8 animate-in fade-in zoom-in-95 duration-500">
       <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs font-normal text-muted-foreground mt-1">AI-powered tracking, message categorization, and smart routing.</p>
          </div>
       ...
       <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden flex flex-col hover:shadow-md transition-shadow">
             <div className="px-5 py-4 border-b border-border bg-slate-50 flex items-center gap-2">
                ...
                <h3 className="text-sm font-normal text-slate-800">Message Triage</h3>
             </div>
             <div className="divide-y divide-border flex-1 bg-white">
                {loading ? ( <div className="p-6 text-center text-xs text-slate-400 font-normal italic">Analyzing text...</div> ) : (
                   data?.message_classification?.map((m, i) => (
                      <div key={i} className="p-5 hover:bg-slate-50/50 transition-colors">
                         <div className="flex flex-wrap justify-between items-start gap-2 mb-2">
                            ...
                            <span className={`px-2 py-0.5 rounded text-[12px] font-normal uppercase tracking-widest border ${getCategoryColor(m.category)}`}>{m.category}</span>
                         </div>
                         <p className="text-sm font-normal text-slate-600">"{m.summary}"</p>
                      </div>
                   ))
                )}
             </div>
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden flex flex-col hover:shadow-md transition-shadow">
             <div className="px-5 py-4 border-b border-border bg-slate-50 flex items-center gap-2">
                ...
                <h3 className="text-sm font-normal text-slate-800">Smart Departmental Routing</h3>
             </div>
             <div className="divide-y divide-border flex-1 bg-white">
                {loading ? ( <div className="p-6 text-center text-xs text-slate-400 font-normal italic">Mapping routes...</div> ) : (
                   data?.department_routing?.map((r, i) => (
                      <div key={i} className="p-5 hover:bg-slate-50/50 transition-colors">
                         <p className="inline-flex items-center gap-1.5 text-[12px] font-normal uppercase tracking-widest text-indigo-700 bg-indigo-50 border border-indigo-200 px-2.5 py-1 rounded shadow-sm mb-2"><Mail className="w-3 h-3"/> Route to: {r.route_to}</p>
                         <p className="text-sm font-normal text-slate-700 leading-snug">{r.message}</p>
                      </div>
                   ))
                )}
             </div>
       ...
       <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden flex flex-col hover:shadow-md transition-shadow">
             <div className="px-5 py-4 border-b border-border bg-slate-50 flex items-center gap-2">
                ...
                <h3 className="text-sm font-normal text-slate-800">Thread Context Management</h3>
             </div>
             <div className="divide-y divide-border flex-1 bg-white">
                {loading ? ( <div className="p-6 text-center text-xs text-slate-400 font-normal italic">Checking history...</div> ) : (
                   data?.conversation_context?.map((c, i) => (
                      <div key={i} className="p-5 hover:bg-slate-50/50 transition-colors">
                         <p className="text-[12px] font-normal uppercase text-slate-400 mb-1.5 tracking-widest">Thread ID: {c.thread_id}</p>
                         <p className="text-sm font-normal text-slate-700 italic border-l-2 border-slate-300 pl-3 leading-relaxed">"{c.context_summary}"</p>
                      </div>
                   ))
                )}
             </div>
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden flex flex-col hover:shadow-md transition-shadow">
             <div className="px-5 py-4 border-b border-border bg-slate-50 flex items-center gap-2">
                ...
                <h3 className="text-sm font-normal text-slate-800">Intelligent Broadcast Logic</h3>
             </div>
             <div className="divide-y divide-border flex-1 bg-white">
                {loading ? ( <div className="p-6 text-center text-xs text-slate-400 font-normal italic">Thinking...</div> ) : (
                   data?.broadcast_suggestions?.map((b, i) => (
                      <div key={i} className="p-5 hover:bg-slate-50/50 transition-colors">
                         <div className="flex flex-wrap justify-between items-center gap-2 mb-2">
                            <p className="text-xs font-normal text-slate-800 uppercase tracking-widest">Target Group</p>
                            <span className="text-[12px] font-normal uppercase tracking-widest bg-blue-50 text-blue-700 border border-blue-200 px-2 py-0.5 rounded">{b.target_group}</span>
                         </div>
                         <p className="text-sm text-slate-600 font-normal leading-relaxed">{b.reason}</p>
                      </div>
                   ))
                )}
             </div>
