    ...
    <div className="mt-8 animate-in fade-in zoom-in-95 duration-500">
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden flex flex-col">
             <div className="px-5 py-4 border-b border-border bg-blue-50 text-blue-900 font-semibold text-sm flex items-center gap-2">
                   ...
                   data?.improvement_recommendations?.map((item, i) => (
                      <div key={i} className="p-5 hover:bg-blue-50/30 transition-colors">
                         <div className="flex items-center gap-2 mb-2">
                            ...
                            <p className="text-xs font-semibold text-slate-800 uppercase tracking-widest">{item.subject}</p>
                         </div>
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden flex flex-col">
             <div className="px-5 py-4 border-b border-border bg-emerald-50 text-emerald-900 font-semibold text-sm flex items-center gap-2">
                   ...
                   data?.teacher_effectiveness?.map((item, i) => (
                      <div key={i} className="p-5 hover:bg-emerald-50/30 transition-colors">
                         <div className="flex items-center justify-between mb-2">
                            <p className="text-sm font-semibold text-slate-800">{item.teacher}</p>
                            <span className={`px-2 py-0.5 rounded text-[12px] font-semibold uppercase tracking-widest border ${item.effectiveness_score >= 80 ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-orange-100 text-orange-700 border-orange-200'}`}>
          ...
          <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden flex flex-col">
             <div className="px-5 py-4 border-b border-border bg-purple-50 text-purple-900 font-semibold text-sm flex items-center gap-2">
                ...
                {loading ? (
                   ...
                   data?.matched_templates?.map((item, i) => (
                      <div key={i} className="p-5 hover:bg-purple-50/30 transition-colors">
                         <span className="inline-block mb-2 px-2.5 py-1 bg-purple-100 text-purple-800 text-[12px] font-semibold uppercase rounded shadow-sm border border-purple-200">
                            {item.type}
                         </span>
