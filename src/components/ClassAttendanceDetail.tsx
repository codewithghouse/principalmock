...
const ClassAttendanceDetail = ({ className, onBack }: ClassAttendanceDetailProps) => {
  ...
  const getPctColor = (pct: number) =>
    pct >= 90 ? '#34C759' : pct >= 70 ? '#FF9500' : '#FF3B30';

  const getStatusStyle = (status: string) => {
    switch (status) {
      case 'Excellent': case 'Good': return 'text-green-600 font-normal';
      case 'Average': return 'text-amber-500 font-normal';
      case 'Warning':  return 'text-amber-600 font-normal';
      case 'Critical': return 'text-red-500 font-normal';
      default: return 'text-foreground font-normal';
    }
  };
  ...
  const exportRegister = () => {
    const html = buildReport({
      ...
      badge: "Attendance",
      heroStats: [
        { label: "Monthly Avg",      value: `${classInfo.monthlyAvg}%`, color: classInfo.monthlyAvg >= 85 ? "#34C759" : "#FFCC00" },
        { label: "Total Students",   value: classInfo.totalStudents },
        { label: "Chronic Absentees", value: classInfo.chronicCount, color: classInfo.chronicCount > 0 ? "#f87171" : "#34C759" },
      ],
      sections: [
        {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-10">
        <div className="animate-spin w-8 h-8 border-4 border-[#1D1D1F] border-t-transparent rounded-full" />
      </div>
    );
  }
  ...
  return (
    <div className="animate-in fade-in duration-500 pb-10">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground mb-2">
        ...
        <span>/</span>
        <span className="text-foreground font-normal">Class Attendance Detail</span>
      </div>
      ...
      <div className="bg-card border border-border rounded-2xl p-6 mb-6 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-normal text-foreground mb-1">{className} Attendance</h1>
          ...
          <div className="flex items-center gap-10">
            <div className="text-right">
              <p className="text-4xl font-normal" style={{ color: getPctColor(classInfo.monthlyAvg) }}>
                {classInfo.monthlyAvg}%
              </p>
              <p className="text-xs font-normal text-muted-foreground">Monthly Average</p>
            </div>
            <div className="text-right">
              <p className="text-4xl font-normal text-[#FF3B30]">{classInfo.chronicCount}</p>
              <p className="text-xs font-normal text-muted-foreground">Chronic Absentees</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-2xl p-8 shadow-sm mb-6">
        <h3 className="text-base font-normal text-foreground mb-6">{monthLabel} Calendar View</h3>

        {/* Day Headers */}
        <div className="grid grid-cols-7 gap-3 mb-3">
          {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(d => (
            <div key={d} className="text-center text-xs font-normal text-muted-foreground uppercase tracking-wider">{d}</div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-7 gap-3">
          {calendarData.map((d, i) => {
            ...
            return (
              ...
              >
                {!d.filler && (
                  <span className="text-sm font-normal" style={{ opacity: 0.8 }}>{d.day}</span>
                )}
                {!d.filler && !d.weekend && d.pct !== null && (
                  <span className="text-xs font-normal">{d.pct}%</span>
                )}
              </div>
            );
          })}
        ...
        <div className="flex items-center gap-6 mt-6 pt-5 border-t border-border">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[12px] font-normal text-muted-foreground">80-100%</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-[#FF9500]" />
            <span className="text-[12px] font-normal text-muted-foreground">70-79%</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-[#FF3B30]" />
            <span className="text-[12px] font-normal text-muted-foreground">Below 70%</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-2xl shadow-sm overflow-hidden mb-6">
        <div className="px-8 py-5 border-b border-border flex items-center justify-between">
          <h2 className="text-base font-normal text-foreground">Student-wise Attendance</h2>
          <div className="flex gap-2">
            <button className="flex items-center gap-2 px-4 py-2 border border-border rounded-xl text-xs font-normal text-foreground hover:bg-secondary transition-colors">
              <Filter className="w-4 h-4" /> Filter
            </button>
            <button
              onClick={exportRegister}
              className="flex items-center gap-2 px-4 py-2 border border-border rounded-xl text-xs font-normal text-foreground hover:bg-secondary transition-colors"
            >
        ...
        {students.length === 0 ? (
          <div className="py-10 text-center text-sm text-muted-foreground font-normal">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border">
                  <th className="px-8 py-4 text-left text-xs font-normal text-[#1D1D1F] uppercase tracking-wider">Student</th>
                  <th className="px-8 py-4 text-left text-xs font-normal text-[#1D1D1F] uppercase tracking-wider">Total Days</th>
                  <th className="px-8 py-4 text-left text-xs font-normal text-[#1D1D1F] uppercase tracking-wider">Present</th>
                  <th className="px-8 py-4 text-left text-xs font-normal text-[#1D1D1F] uppercase tracking-wider">Absent</th>
                  <th className="px-8 py-4 text-left text-xs font-normal text-[#1D1D1F] uppercase tracking-wider">%</th>
                  <th className="px-8 py-4 text-left text-xs font-normal text-[#1D1D1F] uppercase tracking-wider">Status</th>
                  <th className="px-8 py-4 text-left text-xs font-normal text-[#1D1D1F] uppercase tracking-wider">Parent Notified</th>
                </tr>
              </thead>
                  ...
                  <tr key={idx} className="hover:bg-secondary/30 transition-colors">
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-lg bg-secondary flex items-center justify-center text-[12px] font-normal text-muted-foreground">
                          {s.initials}
                        </div>
                        <span className="font-normal text-foreground text-sm">{s.name}</span>
                      </div>
                    </td>
                    <td className="px-8 py-5 text-sm font-normal text-muted-foreground">{s.totalDays}</td>
                    <td className="px-8 py-5 text-sm font-normal text-muted-foreground">{s.present}</td>
                    <td className="px-8 py-5">
                      <span className={`text-sm font-normal ${s.absent >= 5 ? 'text-red-500' : 'text-muted-foreground'}`}>
                        {s.absent}
                      </span>
                    </td>
                    <td className="px-8 py-5">
                      <span className="text-sm font-normal" style={{ color: getPctColor(s.pctVal) }}>{s.pct}</span>
                    </td>
                    <td className="px-8 py-5">
                      <span className={`text-sm ${getStatusStyle(s.status)}`}>{s.status}</span>
                    </td>
                    <td className="px-8 py-5">
                      <span className={`text-sm font-normal ${s.notified === 'Yes' ? 'text-green-500 font-normal' : 'text-muted-foreground'}`}>
                        {s.notified}
                      </span>
                    </td>
                  </tr>
      ...
      {/* ===== ACTION BUTTONS ===== */}
      <div className="flex items-center gap-3">
        <button className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-[#FF3B30] text-white text-sm font-normal hover:bg-red-600 transition-colors shadow-md">
          <MessageSquare className="w-4 h-4" /> Bulk SMS to Absentee Parents
        </button>
        <button
          onClick={exportRegister}
          className="flex items-center gap-2 px-5 py-2.5 rounded-lg border border-border bg-card text-sm font-normal text-foreground hover:bg-secondary transition-colors"
        >
